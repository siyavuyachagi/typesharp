...
export function parseClassesFromFile(content: string, targetAnnotation: string): CSharpClass[] {
    ...
    const annotationRegex = new RegExp(
        `\\[${targetAnnotation}(?:Attribute)?(?:\\(\\s*"([^"]*)"\\s*\\))?(?:\\s*,\\s*[^\\]]+)?\\]`,
        'g'
    );
 ...
 */
function hasUnionAttribute(cleanContent: string, startIndex: number, afterAnnotation: string, matchText: string): boolean {
    // [TypeSharp, Union] — Union inside the same bracket
    if (/\bUnion\b/.test(matchText)) return true;
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
        ...
        if (line === '') continue;
        if (line.startsWith('[')) {
            if (/\[Union\]/.test(line)) return true;
            continue;
        }
        break;
    }
